...
if(expectedCondition.equalsIgnoreCase("enabled")) {
  Assert.assertTrue("loginButton should be enabled", actualCondition);
}else if (expectedCondition.equalsIgnoreCase("disabled")) {
  Assert.assertFalse("loginButton should be disabled", actualCondition);
}
...
Assert.assertEquals("Validated logged in  username",
        expectedUsername,
